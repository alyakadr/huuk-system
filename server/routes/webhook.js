      ...
      if (event.type === "payment_intent.succeeded") {
        ...
        if (relatedBookingIds.length > 0) {
          ...
          const [updateResult] = await connection.query(
            `UPDATE bookings SET payment_status = ?, status = ?, payment_method = ?, is_draft = 0 WHERE id IN (${placeholders})`,
          ...
          const [updateResult] = await connection.query(
            "UPDATE bookings SET payment_status = ?, status = ?, payment_intent_id = ?, payment_method = ?, is_draft = 0 WHERE id = ? AND user_id = ?",
