...
const AddBookingModal = ({ 
  ...
  const getConsecutiveAvailableSlots = useCallback((startTime, date) => {
    ...
    for (let i = startIndex; i < timeSlots.length; i++) {
      ...
      if (slotTaken) {
        break; // Stop counting if we hit an unavailable slot
      }
  ...
  }, [timeSlots, bookings, blockedSlots]);
...
useEffect(() => {
    ...
    const filteredServices = allServices.filter(service => {
      ...
      if (selectedDate === today) {
        ...
        if (timeDiffMinutes > 0 && timeDiffMinutes <= 15) {
          ...
          const fits = service.duration <= totalAvailableTime;
          return fits;
        }
      }
  ...
  }, [allServices, formData.selectedTime, selectedSlot, getMaxServiceDuration]);
          ...
          <form onSubmit={handleSubmit} className="booking-form">
            ...
            <div className="form-group input-field">
              ...
              <div style={{ fontSize: '10px', color: '#666', marginTop: '4px', textAlign: 'center' }}>
                Services: {availableServices.length}, Selected: {formData.service}
              </div>
            </div>
