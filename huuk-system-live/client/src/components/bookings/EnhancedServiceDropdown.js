...
const EnhancedServiceDropdown = ({ 
  ...
  React.useEffect(() => {
    if (value && Array.isArray(services) && services.length > 0) {
      const isValidValue = services.some(service => String(service.id) === String(value));
      if (!isValidValue) {
    ...
    if (value && (!Array.isArray(services) || services.length === 0)) {
      console.warn('🔧 [SERVICE DROPDOWN] Clearing value due to empty services array');
      onChange('');
    }
  ...
  const selectedServiceData = Array.isArray(services) ? services.find(service => String(service.id) === String(value)) : null;
  ...
  const groupedServices = filteredServices.reduce((acc, service) => {
    ...
    if (name.includes('premium') || name.includes('deluxe') || isPremiumService(service.price)) {
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: 280, fontFamily: 'Quicksand, sans-serif', position: 'relative' }}>
      ...
      <StyledSelect
        value={Array.isArray(services) && services.some(service => String(service.id) === String(value)) ? value : ''}
                    ...
                    secondary={
                      <span style={{ 
                        ...
                        <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                          <AccessTime sx={{ fontSize: '12px', color: '#666' }} />
