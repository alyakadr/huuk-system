...
const [signUpUsername, setSignUpUsername] = useState("");
  ...
  // Enhanced authentication check with better session management
  const isLoggedIn = useMemo(() => {
    // Reduced authentication check logging frequency
    if (DEBUG && Math.random() < 0.05) {
      debugLog('[AUTH CHECK] Checking authentication state');
    }
    ...
    // Reduced authentication check logging frequency
    if (DEBUG && Math.random() < 0.05) {
      debugLog('[AUTH CHECK] Profile still loading, returning false');
    }
      return false;
    }
    
    // Primary check: customer tokens (priority)
    const customerToken = localStorage.getItem("customer_token");
    const customerUser = localStorage.getItem("customer_loggedInUser");
    
    // Secondary check: legacy tokens (fallback)
    const legacyToken = localStorage.getItem("token");
    const legacyUser = localStorage.getItem("loggedInUser");
    
    // Reduced authentication check logging frequency
    if (DEBUG && Math.random() < 0.05) {
      debugLog('[AUTH CHECK] Token availability:', {
    ...
    // Check profile context first (most reliable)
    if (profile && profile.id && profile.role === 'customer') {
      const hasValidToken = customerToken || legacyToken;
      debugLog('[AUTH CHECK] Profile context valid, token check:', !!hasValidToken);
      return !!hasValidToken;
    }
    
    // Check customer-specific storage
    if (customerToken && customerUser) {
      try {
        const parsedUser = JSON.parse(customerUser);
        if (parsedUser && parsedUser.id && parsedUser.role === 'customer') {
          debugLog('[AUTH CHECK] Customer token authentication successful');
          return true;
        }
      } catch (error) {
        console.error('[AUTH CHECK] Error parsing customer user data:', error);
      }
    }
    ...
    if (legacyToken && legacyUser) {
      try {
        ...
        if (parsedUser && parsedUser.id && parsedUser.role === 'customer') {
          debugLog('[AUTH CHECK] Legacy token authentication successful');
          return true;
        }
      } catch (error) {
        console.error('[AUTH CHECK] Error parsing legacy user data:', error);
      }
    }
    
    // Reduced authentication check logging frequency
    if (DEBUG && Math.random() < 0.05) {
      debugLog('[AUTH CHECK] No valid authentication found');
    }
    return false;
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/auth/customer/signup`,
        {
...
setTimeout(() => {
  ...
  const handleFormSubmit = async () => {
    ...
    debugLog("🔐 [FORM DEBUG] Auth check - current state:", { isLoggedIn, profileLoading });
  ...
  const updateBookingInList = (editedBooking) => {
    setModalBookings(prev => prev.map(b => b.id === editedBooking.id ? { ...b, ...editedBooking } : b));
    setBookings(prev => prev.map(b => b.id === editedBooking.id ? { ...b, ...editedBooking } : b));
  };
      ...
      >
        ...
        >
          ...
          <div className={styles["homepage-signin-right-section"]}>
            ...
            {loading.signIn && <div>Loading...</div>}
            <form onSubmit={(e) => {
              bookingUtils.handleSignIn(
                e,
                signInPhoneNumber,
                signInPassword,
                setLoading,
                ...
                setSignInPassword,
                () => {
                  const tempFormData = localStorage.getItem('TEMP_BOOKING_FORM_DATA');
                  if (tempFormData) {
                    const parsedFormData = JSON.parse(tempFormData);
                    setPendingBookingData(parsedFormData);
                    setReadyToSubmitBooking(true);
                    localStorage.removeItem('TEMP_BOOKING_FORM_DATA');
                    debugLog('📦 [AUTH] Restored booking form data to pendingBookingData');
                  }
                },
                updateProfile
              );
            ...
            <p>
              <a href="#" className={styles["forgot-password-homepage"]}>
                Forgot password?
              </a>
            </p>
          </div>
