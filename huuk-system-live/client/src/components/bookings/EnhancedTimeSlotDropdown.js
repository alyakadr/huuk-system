...
const EnhancedTimeSlotDropdown = ({ 
  ...
  }, [timeSlots, loading, disabled, errors, value]);
  ...
  const getCurrentTimeStatus = () => {
    ...
    const isPastTenPM = currentHour > 22 || (currentHour === 22 && currentMinute > 0);
    
    return { isPastTenPM, currentHour, currentMinute };
  };
      ...
      >
        ...
        {isPastTenPM && (
          <Box sx={{ p: 2, pb: 1 }}>
            ...
            >
              We're closed! Please book for tomorrow.
            </Alert>
          </Box>
        )}
        ...
        {!isPastTenPM && (!Array.isArray(timeSlots) || timeSlots.length === 0) && (
          <StyledMenuItem disabled>
            <ListItemIcon>
        ...
        {!isPastTenPM && Array.isArray(timeSlots) && timeSlots.length > 0 && filteredSlots.length === 0 && (
