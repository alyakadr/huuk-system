...
const [loading, setLoading] = useState(true);
  ...
  const handleRescheduleSubmit = async (rescheduleData) => {
    try {
      ...
      setAppointments(prev => 
        prev.map(apt => apt.id === rescheduleData.id ? {
          ...apt, 
          booking_date: rescheduleData.newDate, 
          start_time: rescheduleData.newTime
        } : apt)
  ...
  const fetchAppointments = async () => {
    try {
      ...
      console.log('Raw appointments data:', response.data.length, 'total appointments');
      ...
      const validAppointments = response.data.filter(appointment => {
        const hasService = appointment.service_id != null || 
                          (appointment.service && appointment.service !== '' && appointment.service !== null);
        // Exclude draft appointments (is_draft true/1/'1'/true/'true')
        const notDraft = !appointment.is_draft && appointment.is_draft !== 1 && appointment.is_draft !== true && appointment.is_draft !== '1' && appointment.is_draft !== 'true';
        return hasService && notDraft;
      });
      
      console.log('Valid appointments after filtering:', validAppointments.length);
  ...
  useEffect(() => {
    const filterAppointments = () => {
      ...
      if (selectedLocation) {
        ...
        console.log(`Filtered by outlet '${selectedLocation}':`, filtered.length, 'appointments');
      }
      ...
      if (selectedDate) {
        ...
        filtered = filtered.filter(appointment => {
          ...
          if (!appointmentDate.isValid()) {
            console.log('Invalid date for appointment:', appointment.id, appointment.date);
            return false;
          }
        ...
        console.log(`Date filter '${selectedDate}': ${beforeFilter} -> ${filtered.length} appointments`);
      }
...
const handleReschedule = (appointmentId) => {
  ...
  const handleCancel = async (appointmentId) => {
    ...
    const appointmentDetails = appointment ? 
      `\n\nAppointment Details:\n` +
      `• Staff: ${appointment.username || appointment.staffName || 'Unassigned'}\n` +
      `• Service: ${appointment.service}\n` +
      `• Time: ${appointment.start_time} - ${appointment.end_time}\n` +
      `• Date: ${moment(appointment.date).format('DD/MM/YYYY')}` : '';
    
    const confirmCancel = window.confirm(
      `⚠️ WARNING: Are you sure you want to CANCEL this appointment?${appointmentDetails}\n\n` +
    ...
    if (confirmCancel) {
      ...
      const finalConfirm = window.confirm(
        ...
        `Click OK to permanently CANCEL this appointment, or Cancel to go back.`
      );
      
      if (finalConfirm) {
try {
          const response = await axios.put(`${API_BASE_URL}/bookings/staff/appointment/${appointmentId}/status`, {
            status: "cancelled"
          }, {
  ...
  const getActionButtons = (appointment) => {
    if (['Completed', 'Cancelled', 'Absent', 'Rescheduled'].includes(appointment.status)) {
      return (
  ...
  const uniqueDates = Array.from(new Set(appointments.map(a => a.date ? a.date.split('T')[0] : null).filter(Boolean))).sort();
  const uniqueOutlets = Array.from(new Set(appointments.map(a => a.outlet || a.outlet_shortform || a.shortform || a.branch).filter(Boolean)));
  ...
  return (
    <div className="manager-appointment-management">
      ...
      >
        ...
        {outlets.map((outlet) => (
          <button
            ...
            style={{
              background: selectedLocation === (outlet.shortform || outlet.name) ? '#ffa500' : '#232323',
              color: selectedLocation === (outlet.shortform || outlet.name) ? '#1a1a1a' : '#fff',
              fontWeight: 'bold',
              fontFamily: 'Quicksand, sans-serif',
              border: 'none',
              whiteSpace: 'nowrap',
      ...
      <div className="location-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
        ...
        <div className="location-title" style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'flex-start', minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#232323', borderRadius: 6, padding: '6px 12px', marginLeft: 'auto' }}>
              <CalendarIcon />
          <input
            type="date"
            value={selectedDate}
      ...
      >
        <div className="table-header" style={{ gridTemplateColumns: '0.8fr 0.8fr 1fr 1.2fr 1fr', overflow: 'hidden' }}>
          <div className="col-date"><CalendarIcon /><span>DATE</span></div>
          <div className="col-time"><ClockIcon /><span>TIME</span></div>
          <div className="col-staff"><UserIcon /><span>STAFF NAME</span></div>
          <div className="col-service"><ScissorsIcon /><span>SERVICE</span></div>
          <div className="col-actions"><span>ACTIONS</span></div>
        </div>
        <div className="table-body" style={{ flex: 1, overflow: 'hidden', background: 'transparent' }}>
          {Array.from({ length: 4 }).map((_, i) => { // Changed from 5 to 4
            const appointment = paginatedAppointments[i];
            if (appointment) {
              return (
                <div key={appointment.id} className="table-row" style={{ gridTemplateColumns: '0.8fr 0.8fr 1fr 1.2fr 1fr', minHeight: 0, height: '80px', overflow: 'hidden' }}>
                  <div className="col-date" style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {appointment.date ? formatDateForDisplay(appointment.date) : 'N/A'}
                  </div>
                  <div className="col-time" style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {(() => {
                  if (!appointment.start_time || !appointment.end_time || !appointment.date) return 'N/A';
                  ...
                  <div className="col-staff" style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {appointment.username || appointment.staff_username || appointment.user_name ?
                  (appointment.username || appointment.staff_username || appointment.user_name)
                  : (appointment.staffName && appointment.staffName !== '-' && appointment.staffName.trim() !== '' 
                    ? appointment.staffName 
                    : 'Unassigned')}
              </div>
                  <div className="col-service" style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{appointment.service}</div>
                  <div className="col-actions">{getActionButtons(appointment)}</div>
                </div>
              );
                  } else {
              // Render empty row for visual consistency
                  return (
                <div key={`empty-row-${i}`} className="table-row" style={{ gridTemplateColumns: '0.8fr 0.8fr 1fr 1.2fr 1fr', minHeight: 0, height: '80px', overflow: 'hidden', background: 'transparent' }}>
      ...
      {/* Pagination */}
      <div className="pagination" style={{ width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '-15px' }}>
        <button 
          className="btn-prev" 
        ...
        <span className="page-info" style={{ fontWeight: 'bold', fontFamily: 'Quicksand, sans-serif', fontSize: 15 }}>{currentPage}/{totalPages || 1}</span>
        <button 
          className="btn-next" 
