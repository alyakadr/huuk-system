...
exports.getUserBookings = async (req, res) => {
  ...
  try {
    ...
    // This ensures the booking has been fully committed across all database connections
    await new Promise(resolve => setTimeout(resolve, 50));
    
    const [results] = await connection.query(
      `SELECT b.*, o.shortform AS outlet_shortform, s.name AS service_name, s.price, s.duration AS service_duration, u.username AS staff_name,
       ...
       WHERE b.user_id = ? AND (b.is_draft = 0 OR b.is_draft IS NULL)
...
exports.createBooking = async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.query(
      `INSERT INTO bookings (user_id, outlet_id, service_id, staff_id, date, time, customer_name, status, payment_status, is_draft)
...
exports.finalizeBooking = async (req, res) => {
  ...
  try {
    ...
    // Check if booking exists, belongs to user, and is a draft
    const [booking] = await connection.query(
      "SELECT * FROM bookings WHERE id = ? AND user_id = ? AND is_draft = 1",
      [bookingId, req.userId]
    );
    
    if (!booking.length) {
      return res.status(404).json({ message: "Draft booking not found or not authorized" });
    }
    
    // Update the booking to finalize it
    await connection.query(
      "UPDATE bookings SET status = 'Pending', is_draft = 0 WHERE id = ? AND user_id = ?",
      [bookingId, req.userId]
    );
  ...
  try {
    ...
    await connection.query(
      ...
      [
        booking_id,
        "Cancellation",
        `Booking #${booking_id} cancelled on ${formatDateForDb(now)}`,
      ]
    );
    const bookingDetails = {
      id: booking[0].id,
    ...
    // Send SMS cancellation notification if phone number exists (non-blocking)
...
exports.setPayAtOutlet = async (req, res) => {
  ...
  try {
    connection = await pool.getConnection();
    await connection.beginTransaction();
    console.log("Querying booking with:", { booking_id, userId: userId });
    ...
    if (!booking.length) {
      console.error("Booking not found:", { booking_id, userId: userId });
      return res.status(404).json({ message: "Booking not found" });
    }
...
exports.getStaffAppointments = async (req, res) => {
  ...
  try {
    ...
    let whereClause = "WHERE b.staff_id = ? AND b.payment_method IS NOT NULL AND (b.is_draft = 0 OR b.is_draft IS NULL)";
...
exports.getBookingDetails = async (req, res) => {
  ...
  try {
    ...
    const bookingData = {
      ...
      staffName: booking[0].staff_fullname,
  ...
  try {
    ...
    await connection.query(
      ...
      [
        booking_id,
        "Reschedule",
        `Booking #${booking_id} rescheduled to ${formatDateForDb(
          date
        )} at ${time}`,
      ]
    );
    ...
    // Send notification after booking reschedule
    setImmediate(async () => {
      try {
        await sendNotificationAfterBooking('reschedule', {
          id: booking_id,
          user_id: req.userId,
          staff_id: staff_id,
          service_name: booking[0].service_name,
        ...
        console.error('Error sending booking reschedule notification:', notificationError);
      }
    });
