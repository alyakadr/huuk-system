...
router.post("/customer/signin", async (req, res) => {
  ...
  // Import formatPhoneNumber to ensure consistent formatting
  ...
  try {
    ...
    if (results.length === 0) {
      console.log(
        `Sign-in failed: No customer found with phone: ${formattedPhone} (original: ${phone_number})`
      );
      return res
        .status(401)
    ...
    if (!isMatch) {
      console.log(
        `Sign-in failed: Wrong password for phone: ${formattedPhone} (original: ${phone_number})`
      );
      return res
        .status(401)
        .json({ message: "Invalid phone number or password" });
