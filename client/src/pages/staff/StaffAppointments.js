...
const location = useLocation();
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      const response = await api.put(`/bookings/staff/appointment/${selectedAppointment.id}/status`, {
        status: "cancelled"
      }, {
  ...
  const fetchBlockedSlots = async () => {
    try {
      ...
      console.log('Fetching blocked slots for date:', targetDate, 'and staff ID:', userId);
      ...
      if (response.data.success) {
        ...
        if (blockedSlots.length > 0) {
          ...
          console.log('Blocked slots times:', blockedSlots.map(slot => slot.time));
        }
        
        setBlockedSlots(blockedSlots);
      } else {
        console.log('No blocked slots found or error in response:', response.data);
        setBlockedSlots([]);
      }
    } catch (error) {
  ...
  const toggleTimeSlot = async (time) => {
    try {
      ...
      console.log(`Attempting to toggle time slot ${time} on date ${targetDate}`);
    ...
    <div className="w-full min-h-screen font-quicksand text-white">
          ...
          <button 
            ...
            onClick={() => {
              ...
              if (formattedDate === moment().format('YYYY-MM-DD')) {
                ...
                slotsForSelectedDate = todaySlots.length > 0 ? todaySlots : generateTodaySlots();
              } else {
                ...
                console.log(`Generating slots for ${formattedDate} with ${dateAppointments.length} appointments and ${dateBlockedSlots.length} blocked slots`);
                console.log('Blocked slots for this date:', dateBlockedSlots.map(slot => slot.time));
                ...
                while (startTime.isSameOrBefore(endTime)) {
                  ...
                  const isBooked = dateAppointments.some(apt => {
                    if (!apt || apt.status === "-" || apt.start_time === "-" || apt.end_time === "-" ||
                        (apt.status && apt.status.toLowerCase() === "cancelled")) {
                      return false;
                    }
                  ...
                  const isBlocked = dateBlockedSlots.some(slot => slot.time === timeSlot);
              ...
              if (isToday(targetDate.toDate())) {
                ...
                if (!firstAvailableTime) {
                  const availableSlot = slotsForSelectedDate.find(slot => slot.status === 'available');
                  if (availableSlot) {
              ...
              console.log('Available slots for', formattedDate, ':', slotsForSelectedDate.filter(s => s.status === 'available').map(s => s.time));
              console.log('Selected first available time:', firstAvailableTime);
              
              console.log(`Setting selected slot with date: ${formattedDate}, time: ${firstAvailableTime}`);
      ...
      {(() => {
        const filteredBookings = appointments.filter(apt => 
          selectedSlot && 
          extractDateOnly(apt.booking_date) === (selectedSlot.day?.date || selectedSlot.date)
        );
        ...
        const formattedBlockedSlots = blockedSlots.map(slot => {
          ...
          return {
            time: slot.time,
            date: selectedSlot?.day?.date || selectedSlot?.date || moment().format('YYYY-MM-DD'),
        ...
        console.log('- formatted blocked slots:', formattedBlockedSlots.map(s => s.time));
      ...
      >
        ...
        <DialogContent sx={{ pt: 3 }}>
          ...
          <Typography variant="body2" sx={{ mb: 3, color: '#ff6b4a', fontStyle: 'italic' }}>
            This action cannot be undone and the customer will be notified of the cancellation.
          </Typography>
          {selectedAppointment && (
            <Box sx={{ 
              p: 2, 
              ...
              <Typography variant="body2" sx={{ mb: 1 }}>
                <strong>Booking ID:</strong> #{String(selectedAppointment.id).padStart(7, "0")}
              </Typography>
              <Typography variant="body2" sx={{ mb: 1 }}>
                <strong>Service:</strong> {selectedAppointment.service_name || selectedAppointment.service}
              </Typography>
              <Typography variant="body2" sx={{ mb: 1 }}>
                <strong>Date:</strong> {formatDate(selectedAppointment.date || selectedAppointment.booking_date)}
              </Typography>
              <Typography variant="body2" sx={{ mb: 1 }}>
                <strong>Time:</strong> {selectedAppointment.time || selectedAppointment.start_time}
              </Typography>
