...
const EnhancedTimeSlotDropdown = ({
  ...
  const handleChange = (event) => {
    ...
    if (
      ...
      (Array.isArray(timeSlots) &&
        timeSlots.some((slot) => String(slot) === String(selectedValue)))
    ) {
  ...
  const formatTime = (timeSlot) => {
    return new Date(`1970-01-01T${timeSlot}`).toLocaleTimeString("en-GB", {
      hour: "2-digit",
      minute: "2-digit",
      hour12: false,
    });
  ...
  const filteredSlots = React.useMemo(() => {
    if (!Array.isArray(timeSlots)) {
      console.warn(
        "🔧 [TIME SLOT DROPDOWN] timeSlots is not an array:",
        timeSlots,
      );
      return [];
    }
    ...
    const filtered = timeSlots.filter((slot) => {
      try {
        ...
        const matchesSearch =
          timeStr.includes(search) || period.toLowerCase().includes(search);
        ...
        const isNotSelected = !selectedTimes.some(
          (selectedSlot) => String(selectedSlot) === String(slot),
        );
        ...
        if (!shouldInclude && searchTerm) {
          debugLog(
            "🔍 [TIME SLOT FILTERING] Excluding slot due to search:",
            slot,
            "searchTerm:",
            searchTerm,
          );
        ...
        if (
          ...
          debugLog(
            "🔍 [TIME SLOT FILTERING] Excluding slot - already selected:",
            slot,
          );
        }
        ...
        console.error(
          "🔧 [TIME SLOT DROPDOWN] Error filtering slot:",
          slot,
          error,
        );
        return false;
  ...
  const groupedSlots = React.useMemo(() => {
    ...
    return filteredSlots.reduce((acc, slot) => {
      try {
        ...
        console.error(
          "🔧 [TIME SLOT DROPDOWN] Error grouping slot:",
          slot,
          error,
        );
        return acc;
  ...
  const getCurrentTimeStatus = () => {
    ...
    const isToday =
      selectedDateObj && selectedDateObj.getTime() === today.getTime();
  ...
  return (
    ...
    >
      ...
      <StyledSelect
        value={
          Array.isArray(timeSlots) &&
          timeSlots.some((slot) => String(slot) === String(value))
            ? value
            : ""
        }
            ...
            <div key={period}>
              ...
              {slots.map((slot) => {
                ...
                return (
                  ...
                  >
                    <ListItemIcon>{getTimeIcon(slot)}</ListItemIcon>
                    <ListItemText
                      primary={
                        <span
                          style={{
