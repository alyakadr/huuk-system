...
const BookingDetailsModal = ({
  ...
  useEffect(() => {
    if (isOpen && !prevIsOpen) {
      console.log("📝 [MODAL DEBUG] Modal just opened, setting justOpened flag");
      setJustOpened(true);
    }
      ...
      setModalBookings(prev => {
        ...
        if (bookingDetails && Object.keys(bookingDetails).length > 0) {
          const updatedBooking = {
            ...
            customer_name: String(bookingDetails.customer_name || bookingDetails.clientName || "N/A"),
            ...
            payment_method: String(bookingDetails.payment_method || "Stripe"),
            payment_status: String(bookingDetails.payment_status || "Pending"),
            serviceDuration: Number(bookingDetails.serviceDuration) || Number(bookingDetails.duration) || Number(bookingDetails.service_duration) || serviceDuration || 60,
          };
          console.log("📝 [MODAL DEBUG] Initializing modal with bookingDetails:", updatedBooking);
          return [updatedBooking];
        } 
        // Otherwise, use the bookings array if available
        else if (bookings && bookings.length > 0) {
          console.log("📝 [MODAL DEBUG] Initializing modal with bookings array:", bookings);
          return bookings.map(b => ({
            ...
            payment_method: String(b.payment_method || "Stripe"),
            payment_status: String(b.payment_status || "Pending"),
            serviceDuration: Number(b.serviceDuration) || Number(b.duration) || Number(b.service_duration) || serviceDuration || 60,
          }));
        }
  ...
  }, [isOpen, justOpened, bookings, bookingDetails, serviceDuration, isEditingBooking]);

  // Handle editing and updating existing bookings
  useEffect(() => {
    if (isOpen && isEditingBooking && bookingDetails && Object.keys(bookingDetails).length > 0) {
      console.log("📝 [MODAL DEBUG] Updating booking in modal after edit:", bookingDetails);
      ...
      setModalBookings(prevBookings => {
        ...
        if (prevBookings.length === 0) {
          console.log("📝 [MODAL DEBUG] No existing bookings, adding new one:", newBooking);
          return [newBooking];
        }
        
        const bookingIndex = prevBookings.findIndex(b => {
          if (String(b.id).startsWith('temp-') && String(bookingDetails.id).startsWith('temp-')) {
        ...
        if (bookingIndex !== -1) {
          ...
          if (newBooking.staff_name === "N/A" && updatedBookings[bookingIndex].staff_name !== "N/A") {
            newBooking.staff_name = updatedBookings[bookingIndex].staff_name;
            console.log("📝 [MODAL DEBUG] Preserved existing staff name:", newBooking.staff_name);
          }
          updatedBookings[bookingIndex] = newBooking;
          console.log("📝 [MODAL DEBUG] Updated existing booking at index", bookingIndex, ":", newBooking);
          return updatedBookings;
        }
        
        // If not found, add it to the list
        console.log("📝 [MODAL DEBUG] Booking not found in list, adding it:", newBooking);
        return [...prevBookings, newBooking];
      });
    } else if (isOpen && !isEditingBooking && bookingDetails && Object.keys(bookingDetails).length > 0 && isAddingNewBooking) {
    ...
    } else if (isOpen && bookingDetails && Object.keys(bookingDetails).length > 0) {
      // Handle case when modal is open with bookingDetails but not editing or adding
      console.log("📝 [MODAL DEBUG] Modal open with bookingDetails but not editing/adding:", bookingDetails);
      ...
      setModalBookings(prev => {
        ...
        if (prev.length === 0) {
          ...
          console.log("📝 [MODAL DEBUG] Adding booking to empty modal:", newBooking);
          return [newBooking];
        }
        return prev;
      });
  ...
  }, [isOpen, bookingDetails, isEditingBooking, isAddingNewBooking, serviceDuration]);
  ...
  useEffect(() => {
    if (bookingDetails && Object.keys(bookingDetails).length > 0) {
      console.log("📝 [MODAL DEBUG] bookingDetails prop changed:", bookingDetails);
    }
  }, [bookingDetails]);
