...
const EnhancedOutletDropdown = ({ 
  ...
  React.useEffect(() => {
    if (value && Array.isArray(outlets) && outlets.length > 0) {
      ...
      if (!isValidValue) {
        console.warn('[OUTLET DROPDOWN] Clearing invalid value:', value, 'Available outlets:', outlets.map(o => ({id: o.id, name: o.name})));
        onChange('');
      }
    }
    // Also clear value if outlets array becomes empty
    if (value && (!Array.isArray(outlets) || outlets.length === 0)) {
      console.warn('[OUTLET DROPDOWN] Clearing value due to empty outlets array');
      onChange('');
    }
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: 280, fontFamily: 'Quicksand, sans-serif', position: 'relative' }}>
      ...
      <StyledSelect
        value={Array.isArray(outlets) && outlets.some(outlet => outlet.id === value) ? value : ''}
