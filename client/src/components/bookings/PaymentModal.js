...
import { handlePaymentMethodSelection, verifyAuthentication } from "../../utils/bookingUtils";
...
const PaymentModal = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (!auth.isAuthenticated) {
        setPaymentError("You need to be logged in to make a payment. Please sign in and try again.");
      }
    }
  ...
  const handleConfirm = async () => {
    ...
    if (!bookingId) {
      setPaymentError("Booking ID is missing. Please refresh the page and try again.");
      return;
    }
    ...
    if (!effectiveProfile || !effectiveProfile.id) {
      setPaymentError("User profile information is missing. Please sign in again.");
      return;
    }
    ...
    handlePaymentMethodSelection(
      ...
      () =>
        initiatePaymentSession(
          ...
          bookingHistoryRef,
          showSuccessMessage,
          setPaymentError // Use setPaymentError for error display instead of empty function
        ),
      () =>
        handlePayAtOutlet(
          bookingId,
          profileWithRole, // Use the profile with role information
          setLoading,
          setPaymentError,
          setBookingDetails,
          setIsPaymentMethodModalOpen,
  ...
  return (
    ...
    >
      <animated.div style={modalAnimation} className="enhanced-payment-modal-container">
        ...
        {/* Staff Role Alert */}
        {userRole && (userRole === 'staff' || userRole === 'manager' || userRole === 'admin') && (
          <Alert severity="info" sx={{ mb: 2 }}>
            You are logged in as staff. You can process payments on behalf of customers.
          </Alert>
        )}
        ...
        <Box className="payment-actions-section">
          ...
          <Button
            ...
            disabled={loading.payment || loading.paymentInit || !selectedMethod || (authStatus && !authStatus.isAuthenticated)}
            sx={{
