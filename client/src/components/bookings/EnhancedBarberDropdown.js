...
const EnhancedBarberDropdown = ({ 
  ...
  React.useEffect(() => {
    if (value && value !== 'any' && Array.isArray(staff) && staff.length > 0) {
      ...
      if (!isValidValue) {
        console.warn('[BARBER DROPDOWN] Clearing invalid value:', value, 'Available staff:', staff.map(s => ({id: s.id, name: s.username})));
        onChange('');
      }
    }
  ...
  const handleChange = (event) => {
    ...
    if (selectedValue === '' || selectedValue === 'any' || 
        (Array.isArray(staff) && staff.some(member => member.id === selectedValue))) {
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: 280, fontFamily: 'Quicksand, sans-serif', position: 'relative' }}>
      ...
      <StyledSelect
        value={value === 'any' || (Array.isArray(staff) && staff.some(member => member.id === value)) ? value : ''}
        ...
        renderValue={(selected) => {
          ...
          return (
            <span style={{
              ...
              >
                {selectedStaffData ? getInitials(selectedStaffData.username) : '?'}
              </Avatar>
