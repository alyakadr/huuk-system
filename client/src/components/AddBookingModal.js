...
const AddBookingModal = ({ 
  ...
  const snapToNearest30MinSlot = (currentTime) => {
    ...
    const snapTime = moment().hours(hours).minutes(snappedMinutes).format('HH:mm');
  ...
  const findNextAvailableWalkInSlot = (currentTime, date) => {
    for (const slot of timeSlots) {
      ...
      if (slotMoment.isAfter(currentTime) || isSlotUsableForWalkIn(slot, currentTime, date)) {
  ...
  const isSlotTaken = (time, date) => {
    ...
    const isBooked = bookings.some(booking => {
      ...
      if (!booking.end_time && booking.duration) {
        ...
        const overlap = slotTime.isBetween(bookingStart, calculatedEnd, null, '[)');
        if (overlap) {
          console.log(`Slot ${time} is booked: ${booking.service || 'Service'} for ${booking.customer_name || 'Customer'} (${bookingTime}-${calculatedEnd.format('HH:mm')})`);
        }
        return overlap;
      }
      ...
      if (booking.end_time) {
        ...
        const overlap = slotTime.isBetween(bookingStart, bookingEnd, null, '[)');
        if (overlap) {
          console.log(`Slot ${time} is booked: ${booking.service || 'Service'} for ${booking.customer_name || 'Customer'} (${bookingTime}-${booking.end_time})`);
        }
        return overlap;
      }
    ...
    const isBlocked = blockedSlots.some(slot => {
      ...
      console.log('Checking blocked slot:', slot, 'against time:', time, 'date:', date);
      ...
      if (slot.time && slot.date) {
        ...
        if (isMatch) console.log(`Slot ${time} is blocked on ${date} (time+date match)`);
        return isMatch;
      }
      ...
      if (slot.time && slot.day) {
        ...
        if (isMatch) console.log(`Slot ${time} is blocked on ${dayDisplay} (time+day match)`);
        return isMatch;
      }
  ...
  const checkAndSetWalkInOverride = (selectedTime, snapTime, currentTime, selectedDate) => {
    ...
    if (timeDiffMinutes > 0 && timeDiffMinutes <= 15) {
      ...
      if (remainingDuration >= 15) {
        ...
        setWalkInMessage(`Walk-in Override: Slot started ${timeDiffMinutes} minutes ago, ${remainingDuration} minutes remaining`);
        return;
      }
    }
    
    // If we snapped to a different time than originally selected
    if (selectedTime !== snapTime && selectedTime === snapToNearest30MinSlot(currentTime)) {
      setIsWalkInOverride(true);
      setWalkInMessage(`Time snapped to nearest 30-min slot for walk-in booking`);
      return;
    }
  ...
  const lookupBookingHistory = useCallback(async (phoneNumber) => {
  ...
  const lookupCustomerByPhone = useCallback(async (phoneNumber) => {
    ...
    try {
      ...
      const userResponse = await fetch(`${API_BASE_URL}/users/by-phone/${phoneNumber}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      if (userResponse.ok) {
        ...
        if (userData.user) {
          ...
          const historyResponse = await fetch(`${API_BASE_URL}/bookings/by-phone/${phoneNumber}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
  ...
  const getConsecutiveAvailableSlots = useCallback((startTime, date) => {
    ...
    console.log('Blocked slots for date', date, ':', blockedSlots.map(slot => {
    ...
    for (let i = startIndex; i < timeSlots.length; i++) {
      ...
      if (slotTaken) {
        ...
        console.log(`Slot ${currentSlot} is taken - stopping count at ${consecutiveCount} slots`);
        break;
      }
      ...
      if (i === startIndex) {
        ...
        if (date === today) {
          ...
          if (timeDiffMinutes > 0 && timeDiffMinutes <= 15) {
            const remainingDuration = getRemainingSlotDuration(currentSlot, currentTime, date);
            // If less than 20 minutes remaining, this slot can't support full 30-min services
            if (remainingDuration < 20) {
              console.log(`Slot ${currentSlot} has insufficient remaining time (${remainingDuration} min)`);
    ...
    if (consecutiveCount > 0) {
      console.log(`Available slots from ${startTime} on ${date}: ${availableSlots.join(', ')} (${consecutiveCount} slots)`);
    } else {
  ...
  const getMaxServiceDuration = useCallback((startTime, date) => {
...
useEffect(() => {
    if (isOpen) {
      ...
      return () => {if (previousActiveElement) previousActiveElement.focus();};
    }
  }, [isOpen]);
      ...
      // If dynamic time logic is enabled (default behavior for walk-in bookings)
        ...
        if (!smartTime) {
          ...
          if (selectedDate === today) {
            ...
            checkAndSetWalkInOverride(smartTime, snapTime, currentTime, selectedDate);
          } else {
        ...
        // If a specific time was provided, validate and potentially snap it
          ...
          if (selectedDate === today) {
            ...
            else if (!isSlotUsableForWalkIn(smartTime, currentTime, selectedDate)) {
              const snapTime = snapToNearest30MinSlot(currentTime);
              smartTime = getWalkInTimeSlot(currentTime, today);
              checkAndSetWalkInOverride(smartTime, snapTime, currentTime, selectedDate);
            }
            // If slot is usable, still check if it requires override
              ...
              checkAndSetWalkInOverride(smartTime, smartTime, currentTime, selectedDate);
            }
          }
  ...
  }, [isOpen, selectedSlot, timeSlots.length, fetchAllServices, allServices.length]);
  ...
  }, [formData.selectedTime, selectedSlot?.day?.date, selectedSlot?.date, currentUser?.outlet, fetchServicesForSlot, getMaxServiceDuration]);
...
<h3 id="modal-title">ADD NEW BOOKING</h3>
          ...
          <form onSubmit={handleSubmit} className="booking-form">
                ...
                <div className="form-value-display">
                  {(() => {
                    ...
                    if (dateStr) {
                      ...
                      return moment.tz(dateStr, MALAYSIA_TZ).format('DD-MM-YYYY');
                    }
            ...
            {availableDuration > 0 && (
              <div className="form-group info-display" style={{ marginBottom: '10px', justifyContent: 'center' }}>
                <label style={{ fontSize: '12px', color: '#666' }}>Available Duration:</label>
                <div className="form-value-display" style={{ fontSize: '12px', color: '#007bff', fontWeight: 'bold' }}>
            ...
            {isWalkInOverride && walkInMessage && (
              <div className="form-group info-display" style={{ 
                marginBottom: '15px', 
            ...
            <div className="form-group input-field">
              ...
              <div style={{ fontSize: '10px', color: '#666', marginTop: '4px', textAlign: 'center' }}>
                {(() => {
                  const slots = Math.floor(getMaxServiceDuration(formData.selectedTime, selectedSlot?.day?.date || selectedSlot?.date) / 30);
                  if (slots === 0) return 'No available slots';
                  if (slots === 1) return 'Available: 1 slot (30 min services)';
                  if (slots === 2) return 'Available: 2 slots (up to 60 min services)';
              ...
              {(() => {
                ...
                const lastAvailableIndex = startIndex + availableSlotsList.length;
                ...
                return (
                  <div style={{ 
                    ...
                    <div style={{ 
                      ...
                      {availableSlotsList.map((slot, index) => (
                        <div key={slot} style={{
                          fontSize: '9px',
                          padding: '2px 5px',
                          backgroundColor: index === 0 ? '#e3f2fd' : '#f1f8e9',
                          border: '1px solid',
                    ...
                    <div style={{
                      ...
                      <strong>How services are filtered:</strong> Starting from {startTime}, we found {availableSlotsList.length} consecutive available slots
                      {nextUnavailableSlot ? ` (until ${nextUnavailableSlot} which is unavailable)` : ''}.
                      This allows for services up to {availableSlotsList.length * 30} minutes.
                    </div>
                  </div>
                );
              })()}
                    ...
                    >
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ color: '#6c757d', fontSize: '10px' }}>
                          {moment(booking.booking_date || booking.date).format('DD/MM/YYYY')} • {booking.start_time || booking.time}
                        </div>
                      </div>
                      <div style={{
                        fontSize: '10px',
                        color: booking.status === 'completed' ? '#28a745' : '#6c757d',
      ...
      {showOverrideConfirmation && (
        <div className="booking-details-modal" style={{ zIndex: 10001 }}>
          <div className="add-booking-modal-content" style={{ maxWidth: '400px', padding: '20px' }}>
            <div className="booking-details">
              <h3 style={{ color: '#856404', textAlign: 'center', marginBottom: '20px' }}>
              ...
              <div style={{
                ...
                <p style={{ 
                  ...
                  The selected time slot has already started. Do you want to proceed?
                </p>
              </div>
