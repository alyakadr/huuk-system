...
export const verifyAuthentication = () => {
  ...
  console.log("🔑 [AUTH CHECK] Token availability:", {
    customerToken: customerToken ? `${customerToken.substring(0, 10)}...` : "missing",
    ...
    if (isEditingBooking && currentBookingId) {
      ...
      if (response.data && response.data.booking) {
        ...
        if (!staffName && updatedBooking.staff_id) {
          ...
          const matchingStaff = Array.isArray(staff) && staff.find(s => s.id === updatedBooking.staff_id);
          if (matchingStaff) {
            staffName = matchingStaff.name || matchingStaff.username;
            console.log("📝 [SUBMIT] Found staff name from staff array:", staffName);
          }
        }
        ...
        setBookings(prev => prev.map(b => 
          b.id === updatedBooking.id ? { ...b, ...completeBookingDetails } : b
        ));
...
export const handleEditBooking = (
  ...
  console.log("📝 [EDIT] Booking details:", {
    ...
    time: booking.time,
    price: booking.price,
    serviceDuration: booking.serviceDuration || booking.duration || booking.service_duration
  });
...
export const finalizeBooking = async (bookingId) => {
  ...
  try {
    ...
    if (response.data.message && response.data.message.includes("already finalized")) {
    ...
    if (error.response?.data?.message?.includes("already finalized")) {
      ...
      return { id: bookingId, status: error.response?.data?.status || "Confirmed", alreadyFinalized: true };
    }
    
    // Specific error message based on status code
    if (error.response?.status === 401) {
      throw new Error("Authentication failed. Please sign in again to finalize your booking.");
    } else if (error.response?.status === 404) {
      throw new Error("Booking not found. It may have been deleted or already finalized.");
    } else if (error.response?.status === 403) {
      // Check if it's a role-based permission issue
      if (error.response?.data?.message?.includes('role')) {
        throw new Error("You don't have the required role to finalize this booking. Please contact support.");
      } else {
...
export const handlePayAtOutlet = async (
  ...
  console.log("🔍 [PAY AT OUTLET] LocalStorage inspection:", {
    ...
    customerUser: localStorage.getItem("customer_loggedInUser") ? "present" : "missing",
    staffUser: localStorage.getItem("staff_loggedInUser") ? "present" : "missing",
  ...
  try {
    ...
    const isStaff = effectiveUserData.role === 'staff' || 
    ...
    if (combinedData && combinedData.isMultipleBookings) {
      console.log("[PAY AT OUTLET] Processing multiple bookings:", combinedData);
      ...
      if (!actualBookingIds.length || actualBookingIds.some(id => !id || id.toString().startsWith('temp-'))) {
        console.error("❌ [PAY AT OUTLET] Invalid booking IDs in combined data");
        ...
        if (isMultipleBookings) {
          ...
          for (const id of actualBookingIds) {
            try {
              ...
              console.log(`🔄 [PAY AT OUTLET] Attempted to claim booking ${id}: ${claimResult ? "Success" : "Not needed"}`);
            } catch (claimError) {
              console.warn(`⚠️ [PAY AT OUTLET] Non-critical error when claiming booking ${id}:`, claimError.message);
            }
          }
        } else {
          const claimResult = await claimGuestBooking(bookingId);
          console.log(`🔄 [PAY AT OUTLET] Attempted to claim booking ${bookingId}: ${claimResult ? "Success" : "Not needed"}`);
        }
      } catch (claimError) {
        // Just log the error, but continue with the process
        console.warn(`⚠️ [PAY AT OUTLET] Non-critical error when claiming booking:`, claimError.message);
      }
    } else {
      console.log(`🔄 [PAY AT OUTLET] Skipping claim step for staff user (role: ${effectiveUserData.role})`);
    }
    ...
    // Step 3: Verify booking(s) exist
    console.log("🔍 [PAY AT OUTLET] Verifying booking(s) exist...");
    if (isMultipleBookings) {
      try {
        // Get all bookings and verify each one in the list exists
        const bookingResponse = await client.get("/bookings", {
          headers: { Authorization: `Bearer ${token}` },
        });
        
        const existingBookingIds = bookingResponse.data.map(b => b.id.toString());
        const missingBookings = actualBookingIds.filter(id => !existingBookingIds.includes(id.toString()));
        
        if (missingBookings.length > 0) {
          console.error("❌ [PAY AT OUTLET] Some bookings not found:", missingBookings);
          throw new Error(`Some bookings (${missingBookings.join(', ')}) were not found. Please refresh and try again.`);
        }
        
        console.log("✅ [PAY AT OUTLET] All bookings verified:", actualBookingIds);
      } catch (bookingCheckError) {
        console.error("❌ [PAY AT OUTLET] Error verifying bookings:", bookingCheckError);
        throw new Error("Unable to verify bookings. Please try again.");
      }
    } else {
      try {
        // First try to get all bookings and find the one we're looking for
        const bookingResponse = await client.get("/bookings", {
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        if (!existingBooking) {
          console.error("❌ [PAY AT OUTLET] Booking not found in user's bookings:", bookingId);
          console.log("📋 [PAY AT OUTLET] Available bookings:", bookingResponse.data.map(b => ({ id: b.id, customer: b.customer_name })));
          
          // Try to get the specific booking details as a fallback
          console.log("🔄 [PAY AT OUTLET] Trying to get specific booking details...");
          try {
            const specificBookingResponse = await client.get(`/bookings/booking-details/${bookingId}?debug=true`, {
              headers: { Authorization: `Bearer ${token}` },
            });
            
            if (specificBookingResponse.data) {
              console.log("✅ [PAY AT OUTLET] Found booking via direct lookup:", {
                id: specificBookingResponse.data.id,
                customer: specificBookingResponse.data.customer_name
              });
              // Continue with the booking we found
            } else {
              throw new Error("Booking not found via direct lookup");
            }
          } catch (specificLookupError) {
            console.error("❌ [PAY AT OUTLET] Failed to find booking via direct lookup:", specificLookupError);
    ...
    if (isMultipleBookings) {
      ...
      console.log("🚀 [PAY AT OUTLET] Setting pay-at-outlet for multiple bookings:", {
        bookingIds: actualBookingIds,
        email: emailForBackend
      });
      
      // Add detailed logging for the API call
      console.log("📡 [PAY AT OUTLET] Making API call to set-multiple-pay-at-outlet...");
      
      // Use the already imported setMultiplePayAtOutlet function
      const apiResponse = await setMultiplePayAtOutlet(actualBookingIds, emailForBackend, userData.id);
      console.log("✅ [PAY AT OUTLET] API call successful:", apiResponse.data);
      
      // Step 5: Update UI state for multiple bookings
      console.log("🔄 [PAY AT OUTLET] Updating UI state for multiple bookings...");
    ...
    if (isMultipleBookings) {
      console.log("🔄 [PAY AT OUTLET] Finalizing multiple draft bookings after payment method set");
      try {
        ...
        for (let i = 0; i < actualBookingIds.length; i++) {
          ...
          try {
            ...
            if (i > 0) {
              console.log(`🔄 [PAY AT OUTLET] Adding delay before finalizing booking ${id}`);
            ...
            if (finalizeError.message.includes("already finalized")) {
              console.log(`✅ [PAY AT OUTLET] Booking ${id} was already finalized`);
            } else {
              console.error(`❌ [PAY AT OUTLET] Error finalizing booking ${id}:`, finalizeError);
            }
          }
        }
        ...
        console.error("❌ [PAY AT OUTLET] Error finalizing bookings:", finalizeError);
        
        // Show a warning but don't fail the entire process
        if (showErrorMessage) {
          showErrorMessage("Your booking payment method was set, but there was an issue finalizing the bookings. Please check your booking status.");
        }
      }
    } else {
      ...
      try {
        ...
        // Show a warning but don't fail the entire process
        if (showErrorMessage) {
          showErrorMessage("Your booking payment method was set, but there was an issue finalizing the booking. Please check your booking status.");
        }
      }
    }
    ...
    setTimeout(() => {
      if (showSuccessMessage) {
        if (isMultipleBookings) {
          showSuccessMessage(`Payment method set to Pay at Outlet successfully for ${actualBookingIds.length} bookings! All bookings finalized.`);
        } else {
...
export const claimGuestBooking = async (bookingId) => {
  try {
    const token = getAuthToken();
    if (!token) {
      console.warn(`Skipping claim for booking ${bookingId}: No authentication token found`);
    ...
    if (userData && (userData.role === 'staff' || userData.role === 'manager' || userData.role === 'admin')) {
      console.log(`ℹ️ Booking ${bookingId} claim skipped: Staff users don't need to claim bookings`);
      return true; // Return true to indicate success (no need to claim)
    }

    // Only proceed with claim if user is a customer
    const response = await client.patch(`/bookings/${bookingId}/claim`, {}, {
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    if (error.response?.status === 404) {
      console.log(`ℹ️ Booking ${bookingId} is not a guest booking or doesn't exist.`);
      return false;
    }
    ...
    if (error.response?.status === 403) {
      ...
      if (error.response?.data?.message?.includes('role')) {
        console.log(`ℹ️ User doesn't have customer role to claim booking ${bookingId}.`);
      } else {
        console.log(`ℹ️ User doesn't have permission to claim booking ${bookingId}.`);
      }
      return false;
    }
  ...
  client
    ...
    .then((response) => {
      ...
      if (availableSlots.length === 0 && isFutureDate) {
        console.log("📅 [FUTURE DATE] No slots returned for future date, generating default slots");
        ...
        console.log("📅 [FUTURE DATE] Generated default slots:", availableSlots);
      }
    ...
    .catch((error) => {
      ...
      if (isFutureDate) {
        console.log("📅 [FUTURE DATE] Error fetching slots for future date, generating default slots");
